...
output = root / "linkverse-platform/recommendation-experiments/runs/trade-ranker-runtime-v2-20260908"
settings = {"before": str(base), "after": str(candidate), "port": 18085, "container_metrics": True,
            "metric_containers": ["linkverse-mvp-recommendation-ranker-20260908"], "container_memory_gib": 6,
            "purpose": "ISOLATED_LINUX_HTTP_VALIDATION"}
...
def wait_version(client, version):
    ...
    while time.monotonic() - started < 30:
        try:
            ...
            record("readiness-probes", error_type=type(error).__name__)
            continue
...
with RunRecorder(output, settings, root):
    ...
    with httpx.Client(base_url="http://127.0.0.1:18085", trust_env=False, timeout=10) as client:
        for name, bundle in (("before", base), ("after", candidate)):
            ...
            model = models[name]
            users = sorted(model.user_vocabulary)[:10] + [hashlib.sha256(f"runtime-cold-{i}".encode()).hexdigest() for i in range(5)]
            for index, user in enumerate(users):
                for scene in ("HOME", "DETAIL", "CART"):
                    for count in (100, 300):
                        ...
                        payload = {"request_id": uuid.uuid4().hex, "user_key": user, "domain": "trade",
        ...
        def request(index):
            body = {**payload, "user_key": users[-1], "request_id": uuid.uuid4().hex}
