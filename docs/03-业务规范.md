...
> [!IMPORTANT]
> 本文的 Trade 支付表与五单元边界属于长期蓝图。当前阶段 1～2 执行 [`04-MVP开发步骤与注意事项.md`](04-MVP开发步骤与注意事项.md) 与 [`ADR-0001`](adr/0001-MVP独立Payment服务边界.md)：Payment 独占 `linkverse_mvp_payment` Schema，Trade 不得读写 Payment 表，两者使用受认证内部 API 和版本化事件协作；Forum 与 Recommendation 延期。本文中与该 MVP 边界冲突的 Trade 支付表、同事务更新和迁移顺序，在 MVP 内不适用。
...
| `outbox_event` | 身份领域事务出站事件 | `event_id` 唯一；索引 `(status, available_at, id)` |
...
2. 由 `linkverse-platform/tools/` 使用旧源只读账号导入脱敏快照，并生成数据质量报告。
3. 新服务不得运行时读取旧表，也不得双写新旧 MySQL、Redis 或 ES。
4. 按 Identity、Trade、Forum、行为事件的顺序迁移，统一维护旧新 ID 映射和删除标记。
5. 正式切换前停止旧写，执行最终增量迁移并对账主键、状态、金额、库存和计数。
6. 旧库后续保持只读；归档或删除必须单独获得用户批准。
