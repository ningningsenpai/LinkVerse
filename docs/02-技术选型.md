...
> [!IMPORTANT]
> 本文保留长期 Gateway、Identity、Trade、Forum 和 Python Recommendation 的技术蓝图。当前阶段 1～2 以 [`04-MVP开发步骤与注意事项.md`](04-MVP开发步骤与注意事项.md) 为覆盖层：只建设 Gateway、Identity、Trade、Payment 四个 Java 单元，Forum、Recommendation、Elasticsearch 和完整观测栈延期。当前 Reactor 采用扁平模块 `platform-core`、`platform-starter-web`、`platform-starter-security`、`platform-starter-observability`、`platform-starter-messaging`；Payment 独立边界见 [`ADR-0001`](adr/0001-MVP独立Payment服务边界.md)。
...
- `linkverse-identity`：Spring Security、Spring Authorization Server、MyBatis-Plus、MySQL、Redis、RabbitMQ；通过本地 Outbox 发布身份事件。
- `linkverse-trade`：Spring MVC、MyBatis-Plus、MySQL、Redis、RabbitMQ；ES 仅作为可选商品搜索投影。
- `linkverse-payment`：Spring MVC、MyBatis-Plus、MySQL、RabbitMQ；支付事实、回调、退款和 Outbox 保持在独立 Schema 内。
- `linkverse-forum`：长期规划使用 Spring MVC、MyBatis-Plus、MySQL、Redis、RabbitMQ；当前 MVP 不创建该服务。

业务服务的持久化基线统一为 MyBatis-Plus `3.5.17`。标准单表操作使用 `BaseMapper`，条件更新、锁定读、消费去重和 Outbox 抢占使用显式 Mapper SQL，具体决策见 [ADR-0002：全局采用 MyBatis-Plus 持久化](adr/0002-全局采用MyBatis-Plus持久化.md)。
